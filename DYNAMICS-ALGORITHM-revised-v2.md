...
20. [Implementation Reference](#20-implementation-reference)
    - [10.4 Stress-Scenario Method Selection](#104-stress-scenario-method-selection)
    - [10.5 Divergence Handling Rules](#105-divergence-handling-rules)
    - [11.5 Severe-War Transmission Template (UI Path View)](#115-severe-war-transmission-template-ui-path-view)
    - [11.6 Aggregation & Interpretation Guardrails](#116-aggregation--interpretation-guardrails)
    - [18.3 Scenario / Period / Method Decision Matrix](#183-scenario--period--method-decision-matrix)
    - [20.4 UI Display Guardrails](#204-ui-display-guardrails)
    - [20.5 Dashboard Menu Map](#205-dashboard-menu-map)
    - [20.6 Dashboard > System Dynamics Panel Contract](#206-dashboard--system-dynamics-panel-contract)
...
### 10.4 Stress-Scenario Method Selection

```
IF preset ∈ { severeWar, hormuzBlockade, fullScaleConflict }:
  primary   = LP(period = warEra OR full)
  secondary = ARDL(period = full)        ← structural cross-check
  explain   = Gaussian(period = same)    ← visual narrative / quick scan
ELSE IF purpose = current baseline outlook:
  primary   = ARDL(period = full OR recent if feasible)
  fallback  = Gaussian
ELSE IF purpose = presentation / committee pack:
  primary   = all 3 methods side-by-side
```

| Scenario / Use Case          | Period Priority        | Primary Method | Secondary Method | Rationale |
|-----------------------------|------------------------|----------------|------------------|-----------|
| 전쟁 재발 / 공급충격 재현     | `warEra` → `full`      | LP             | ARDL             | 대형 비선형 충격에서 horizon별 직접 추정이 더 강건 |
| 장기 구조 추정               | `full`                 | ARDL           | LP               | 충분한 표본에서 평균 구조와 계수 안정성 확인 가능 |
| 최근 추세 확인               | `recent` → `full`      | Gaussian       | LP (low-h only)  | 최근 구간은 표본 부족으로 ARDL 불안정 가능 |
| 설명용 경로 시각화           | any                    | Gaussian       | —                | 경로별 전달 로직을 직관적으로 보여주기 쉬움 |

### 10.5 Divergence Handling Rules

```
Case A: Gaussian / ARDL / LP all same sign
  → high conviction

Case B: 2 methods same sign, 1 differs
  → moderate conviction
  → inspect sample period, large-shock nonlinearity, and fallback status

Case C: all differ materially
  → high uncertainty
  → escalate to Monte Carlo + sensitivity analysis
```

| Pattern                   | Interpretation                         | Recommended Action |
|--------------------------|-----------------------------------------|--------------------|
| 3개 방법 모두 동일 방향     | 구조적으로 일관된 신호                   | 예산 / 헤지 / 분양전략 조정 검토 |
| 2:1 분리                 | 평균 구조와 스트레스 구조가 충돌         | 소수 의견의 표본·시차 가정 점검 |
| 완전 불일치              | 모델 의존성 과다, 불확실성 높음          | Monte Carlo, 추가 시나리오, 보수적 의사결정 |
...
### 11.5 Severe-War Transmission Template (UI Path View)

The current dashboard preset labelled **"심각한 전쟁"** should be interpreted as a **two-axis stress map** rather than a single scalar outcome.

| Path | Transmission Chain | Dominant Link | Terminal KPI | Sign | Interpretation |
|------|--------------------|---------------|--------------|------|----------------|
| 1 | 유가 급등 → 에너지비 직접전가 → 건설자재 비용 | energy pass-through | 건설공사비 `+18.0%` | + | 가장 큰 원가 상승 축 |
| 2 | 공급망 차질 → 원자재 부족 / 수급불균형 → 자재가격 | supply bottleneck | 건설공사비 `+6.0%` | + | 전쟁형 공급충격의 2차 비용 축 |
| 3 | 조달금리 상승 → PF 대출금리 / 스프레드 → 금융비용 | financing spread | 건설공사비 `+1.0%` | + | 금융채널은 존재하나 상대적으로 약함 |
| 4 | 대출금리 상승 → 주택수요 둔화 → 미분양 증가 | demand / absorption | 사업성 `-11.8%` | − | 비용 상쇄가 아니라 수익성 붕괴로 나타남 |

```
Key interpretation:
  cost axis        = Path 1 + Path 2 + Path 3   → construction cost inflation
  feasibility axis = Path 4                      → sales / absorption / project viability stress

Therefore:
  weak demand does NOT necessarily offset high construction cost.
  It often appears as lower feasibility, higher unsold inventory, and PF stress.
```

### 11.6 Aggregation & Interpretation Guardrails

```
Do NOT mechanically sum:
  right-end path boxes  ≠  headline totals at bottom bar

Possible reasons:
  1. different aggregation spaces (path-end KPI vs system-level cumulative total)
  2. nonlinear compounding across horizons
  3. overlap removal / normalization in the attribution engine
  4. mixed cost-axis and demand-axis reporting
```

| Guardrail | Meaning | Implementation Hint |
|-----------|---------|---------------------|
| Cost vs demand 분리 | 원가 상승과 사업성 하락은 다른 결과 변수 | UI에서 서로 다른 색과 소계로 표시 |
| Path-end vs total 분리 | 말단 박스 숫자는 경로별 종착 기여치 | 하단 총계는 별도 누적·정규화 산식 표기 |
| `r` and `β` 주석 | 상관·추정계수는 설명용 / 예측용 | 구조적 인과로 과도 해석 금지 |
| Fallback 표시 | ARDL / LP 추정 불가 시 Gaussian 대체 | 활성 방법론 badge + fallback note 추가 |

**Interpretation rule of thumb**

- `r` or lag-correlation: co-movement diagnostic
- `β` in ARDL / LP: estimated predictive response under selected sample
- Neither alone proves structural causality
...
### 18.3 Scenario / Period / Method Decision Matrix

| Decision Context | Preferred Period | Preferred Method | Why |
|------------------|------------------|------------------|-----|
| 전쟁 재발 스트레스 테스트 | `warEra` + `full` cross-check | LP | 극단 충격의 비선형 반응을 직접 추정 |
| 위원회 / 투자심의 보고 | `full` | all 3 methods | 방법론 간 수렴·괴리 자체가 의사결정 정보 |
| 현재 베이스라인 운영전망 | `full` or feasible `recent` | ARDL | 평균 구조에 기반한 실무형 계수 해석 가능 |
| 빠른 사전 진단 / 데모 | any | Gaussian | 계산이 빠르고 경로 설명력이 높음 |

```
Recommended display order for severe-war preset:
  1. LP (primary stress result)
  2. ARDL full-sample cross-check
  3. Gaussian explanatory path map
```
...
### 20.4 UI Display Guardrails

| UI Element | Required Disclosure | Reason |
|------------|---------------------|--------|
| 전달 메커니즘 탭 | 활성 분석기간 / 활성 방법론 | 사용자가 현재 숫자의 추정 기반을 즉시 인지 |
| 경로별 우측 종착값 | `path-end contribution` 라벨 | 총계와 동일한 숫자로 오해 방지 |
| 하단 총계 바 | 누적 방식 / 정규화 여부 footnote | 경로합과 총계의 차이를 설명 |
| 방법론 badge | `estimated`, `fallback`, `unavailable` | 소표본·추정불가 상황을 숨기지 않음 |
| 수요/사업성 카드 | 별도 컬럼 및 색상체계 | 건설비와 사업성을 분리 표시 |

```
Example UI note:
  "Path-end values show terminal route contributions.
   Bottom headline totals are system-level cumulative effects and are not directly additive."
```


### 20.5 Dashboard Menu Map

| Route / Menu | UI Location | Primary Purpose | Required Content |
|--------------|-------------|-----------------|------------------|
| `Dashboard` | 상단 글로벌 내비게이션 | 실시간 위기현황, 핵심 지표, 뉴스, 압력상태 요약 | 실시간 지표 카드, 위험등급, 뉴스 피드, 주요 알림 |
| `System Dynamics` | 상단 글로벌 내비게이션 또는 Dashboard 진입 CTA | 충격이 건설비·사업성으로 전달되는 구조를 경로 중심으로 설명 | CLD, Stress Map, Stock & Flow, Pass-through legend |
| `Methodology` | 상단 글로벌 내비게이션 | Gaussian / ARDL / LP의 추정 방식과 사용 규칙 설명 | 추정식, 기간별 제약, fallback 규칙, 해석 원칙 |
| `Impact Simulation Engine` | Dashboard 내부 상세 모듈 | 시나리오 프리셋과 탭 기반 분석 수행 | 프리셋, 입력값, 탭 UI, 경로별 결과, 하단 총계 |

**Navigation principle**

```
Global Nav:
  Dashboard | System Dynamics | Methodology

Dashboard detail CTA:
  Dashboard → Impact Simulation Engine → [파라미터 설정 / 민감도 분석 / MONTE CARLO / 전달 메커니즘]
```

The standalone **System Dynamics** page and the in-dashboard **전달 메커니즘** tab must use the same vocabulary, scenario labels, and disclosure rules so that users do not see two different interpretations of the same shock.

### 20.6 Dashboard > System Dynamics Panel Contract

#### 20.6.1 Required Tab Set

| Tab | Purpose | Minimum Required Elements |
|-----|---------|---------------------------|
| `파라미터 설정` | 시나리오 입력·프리셋 조정 | 프리셋 버튼, 유가/금리/공급망 등 핵심 입력, 적용 버튼 |
| `민감도 분석` | 단일 변수 변화에 대한 3M/6M/12M 민감도 확인 | 변수 선택, 변화폭 축, 방법론 badge, 민감도 표/차트 |
| `MONTE CARLO` | 확률분포 기반 tail risk 확인 | 분포 가정, draw 수, percentile band, stress probability |
| `전달 메커니즘` | 충격이 건설비·사업성으로 전달되는 경로 설명 | 경로 lane, path-end value, 총계, 축 분리 주석 |

#### 20.6.2 전달 메커니즘 탭 — Mandatory Layout

```
Header:
  [Scenario preset buttons]
  [Active period chip] [Active method chip] [Status: estimated/fallback/unavailable]

Body:
  Path 1  유가 급등 → 에너지 비용 직접전가 → 건설자재 비용 상승 → 건설공사비
  Path 2  공급망 차질 → 원자재 부족/수급불균형 → 자재가격 상승 → 건설공사비
  Path 3  금리/조달비용 → PF 대출금리·스프레드 → 금융비용 상승 → 건설공사비
  Path 4  대출금리 상승 → 주택수요 둔화 → 미분양 증가 → 사업성

Footer:
  총 건설비 영향 / 총 수요 영향 / 적용 분석기간
  footnote: path-end values are not directly additive to headline totals
```

#### 20.6.3 Severe-War Preset Binding

For the **심각한 전쟁** preset, the dashboard panel must explicitly bind to the four-path stress map below.

| Path | Axis | Headline Reading |
|------|------|------------------|
| Path 1 | 원가 축 | 유가 급등이 에너지비와 자재비를 통해 공사비를 크게 밀어 올림 |
| Path 2 | 원가 축 | 공급망 훼손이 수급불균형과 자재가 급등으로 연결 |
| Path 3 | 원가 축 | 금리·조달채널은 존재하나 비용 상승 기여는 상대적으로 제한적 |
| Path 4 | 사업성 축 | 금리상승은 원가 하락보다 미분양과 사업성 악화로 먼저 반영 |

#### 20.6.4 Disclosure Rules for Dashboard Embedding

| Field | Display Rule |
|-------|--------------|
| Scenario | 프리셋명과 외생 입력값을 동시에 표시 |
| Method | `Gaussian`, `ARDL`, `LP`, `ALL` 중 현재 표시 기준 명시 |
| Period | `full`, `warEra`, `postWar`, `recent` 중 하나를 chip으로 표시 |
| Status | `estimated`, `fallback`, `unavailable`를 숨기지 않고 badge로 표기 |
| Totals | `path-end` 와 `headline total`을 서로 다른 라벨·행으로 분리 |

This menu contract is needed because the dashboard user first sees **executable scenario controls**, while the standalone System Dynamics page provides **conceptual structure**. Both surfaces must resolve to the same semantic model.
